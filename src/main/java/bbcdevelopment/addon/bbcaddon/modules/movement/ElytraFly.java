...
public class ElytraFly extends BBCModule {
    ...
    @EventHandler
    private void onElytraMove(TickEvent.Post event) {
        ...
        if (length > maxBoostSpeed.get() && limit.get()) {
            ...
            if (mode.get() == Mode.Boost && (mc.player.getPitch() >= minPitch.get() && safePitch.get() || !safePitch.get()))
                mc.player.addVelocity(speedX, 0, speedZ);
            else {
                targetY.get();
            }
        }
